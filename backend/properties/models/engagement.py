...
# backend/properties/models/engagement.py
import uuid
from django.db import models
from django.utils.timezone import now
...
class ListingEngagement(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    listing = models.OneToOneField("PropertyListing", on_delete=models.CASCADE, related_name="engagement")
    views = models.PositiveIntegerField(default=0)
    inspections = models.PositiveIntegerField(default=0)
    ...
    def __str__(self):
        return f"Engagement for {self.listing.title}"
