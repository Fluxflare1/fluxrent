...
export default function DisputeModal({ auditUid, txn, onClose, onCreated }: Props) {
  const [reason, setReason] = useState("");
  const [submitting, setSubmitting] = useState(false);
  const { toast } = useToast();

  if (!auditUid && !txn) return null;
  ...
  async function submit() {
    ...
    try {
      await apiFetch(ENDPOINTS.finance.disputes, {
        method: "POST",
        body: JSON.stringify({
          transaction: auditUid || txn?.reference,
          reason,
          evidence: {},
        }),
      });
  ...
  const reference = txn?.reference || auditUid;

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-lg p-6 w-full max-w-xl">
        <h3 className="text-lg font-semibold">Dispute Transaction</h3>
        <p className="text-sm text-slate-500 mt-1">Transaction: {reference}</p>
