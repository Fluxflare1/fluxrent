  ...
  async function loadListings(currentFilters: ListingFilters = {}) {
    ...
    try {
      ...
      try {
        ...
        const res = await fetch(`${API_BASE_URL}/properties/listings/?${params}`);
                  ...
                  <li key={property.id} className="border p-4 rounded-lg hover:shadow-md transition-shadow">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        {property.address && <p className="text-xs text-gray-500 mt-1 truncate">{property.address}</p>}
                      </div>
                      {property.media?.[0]?.file && (
                        <img src={property.media[0].file} alt={property.title} className="w-20 h-20 object-cover rounded ml-4" />
                      )}
                    </div>
                  </li>
                ))}
